...
if [ "$TOMCAT_HOME" = "" ] ; then
    if [ -d ${WATCHDOG_HOME}/../tomcat ] ; then
        TOMCAT_HOME=${WATCHDOG_HOME}/../tomcat
...
CLASSPATH=${TOMCAT_HOME}/lib/servlet.jar:$CLASSPATH
...
if [ "${default}" = all ] ; then
    java -Dtomcat.home=${TOMCAT_HOME} org.apache.tools.ant.Main \
        -Dport=${PORT} -Dhost=${HOST} -Dwatchdog.home=${WATCHDOG_HOME} \
        -f ${WATCHDOG_HOME}/conf/runtest.xml \
        startup servlet-test jsp-test shutdown
fi
...
if [ "${default}" = jsp ] ; then
    java -Dtomcat.home=${TOMCAT_HOME} org.apache.tools.ant.Main \
        -Dport=${PORT} -Dhost=${HOST} -Dwatchdog.home=${WATCHDOG_HOME} \
        -f ${WATCHDOG_HOME}/conf/runtest.xml \
        startup jsp-test shutdown
fi

if [ "${default}" = servlet ] ; then
    java -Dtomcat.home=${TOMCAT_HOME} org.apache.tools.ant.Main \
        -Dport=${PORT} -Dhost=${HOST} -Dwatchdog.home=${WATCHDOG_HOME} \
        -f ${WATCHDOG_HOME}/conf/runtest.xml \
        startup servlet-test shutdown
fi
